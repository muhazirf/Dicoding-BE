...
const { mapDBToModelPlaylistSong, mapDBToModelPlaylistActivity } = require('../../../utils')
...
class PlaylistService {
  ...
  async getActivity (playlistId) {
    ...
    return mapDBToModelPlaylistActivity(playlist[0].id, rows)
  }
  ...
  async getPlaylistSong (playlistId) {
    ...
    const querySong = {
      text: `SELECT song.id, song.title, song.performer FROM playlist_songs
    ...
    return mapDBToModelPlaylistSong(resultPlaylist.rows[0], resultSongs.rows)
  }

  async addPlaylistSong (playlistId, songId, userId) {
    const songIdfromDatabase = await this._songService.getSongById(songId)
